...
bool Meta::GeneOnly = false; // skip single variant test
...
//printf("i=%d, common_chr,pos:%s,%d\n",i,common_chr[i].c_str(), common_pos[i]);
...
//printf("i=%d,idx=%d,chr=%s,pos=%d,covfile=%s\n",i,idx,common_chr[idx].c_str(),common_pos[idx],covFilename.c_str());
...
void Meta::PoolSummaryStat(GroupFromAnnotation & group,FILE * log)
{
	...
	if (GeneOnly) {
		printf("\nWith option --geneOnly, skip single variant meta analysis!\n");
	}
	else {
	...
	if (GeneOnly) // no need for GC or plot
		return;		
	...
	if(cond!="")
	{
			...
			if ( pvalueAll_cond.size == 0 )
				error("No available p value for conditional variants. Something is wrong!");
			double GC1 = GetGenomicControlFromPvalue(pvalueAll_cond);
			demo1="GC=";
			demo1 += GC1;
			double GC2;
			if (pvalue1_cond.size == 0)
				printf("\nWarning: no maf<0.01 conditional variants. This group GC=0!");
			else
				GC2 = GetGenomicControlFromPvalue(pvalue1_cond);
			demo2="GC=";
			demo2 += GC2;
			double GC3;
			if (pvalue5_cond.size == 0)
				printf("\nWarning: no maf<0.05 conditional variants. This group GC=0!");
			else
				GC3 = GetGenomicControlFromPvalue(pvalue5_cond);
			demo3="GC=";
			demo3 += GC3;
				...
				for(int m=0;m<count;m++)
			{
				...
				for(int s=m;s<count;s++)
				{
					...
					else
					{
						...
						if(flip!=-1)
							factor=-1.0;
//printf("pos=%d,cov=%g,N=%d\n",pos[s].AsInteger(),markerscov[p].AsDouble(),SampleSize[study]);
				...
				if(cond!="")
				{
					...
					{
//printf("preGx ps[m] = %d, common_pos = %d\n",pos[m].AsInteger(), common_pos[commonVar_study[study][s]]);
						if(pos[m].AsInteger()<common_pos[commonVar_study[study][s]]) { // direct fill in
							...
							if(p==-1)
								continue;
//printf("GX cov=%g, m=%d, s=%d, p=%d\n", markerscov[p].AsDouble(), m, s, p);
							...
							if (p==-1)
								continue;
//printf("GX cov=%g, m=%d, s=%d, p=%d\n", new_markerscov[p].AsDouble(), m, s, p);
