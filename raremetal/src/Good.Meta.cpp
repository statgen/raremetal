...
void Meta::UpdateExcludedMarker(int & skip_count,StringIntHash & SNPexclude,int & study, String & chr_pos,int filter,String markername,FILE * log,StringIntHash & directionByChrPos,StringArray & refalt)
{
   ...
   if(skip_count==1)
      printf("Warning: variants have been excluded from analysis due to un-matched alleles. Pleas check log file for details.\n");
   if(filter==0)
   {
      fprintf(log,"Warning: variant %s from study %s failed to pass HWE or CALLRATE filter and it is excluded from meta analysis.\n",markername.c_str(),studies[study].c_str());
      return;
   }
   if(filter==1)
   {
      fprintf(log,"Warning: variant %s from study %s has at least one allele missing but is polymorphic; and is excluded from meta-analysis.\n",markername.c_str(), studies[study].c_str());
      return;
   }
   if(filter==2)
   {
      int idx = directionByChrPos.Integer(chr_pos);
      fprintf(log,"Warning: variant %s from study %s is excluded due to unmatched alleles (expecting to see %s).\n",markername.c_str(),studies[study].c_str(),refalt[idx].c_str());
      return;
   }
}
...
void Meta::PoolSummaryStat(GroupFromAnnotation & group,FILE * log)
{
   ...
   for(int i=0;i<directions.Length();i++)
      if(directions[i].Length()<studies.Length())
	 directions[i]+='?';
...
void Meta::Run(GroupFromAnnotation & group,FILE * log)
{
   ...
   for(int g=0;g<group.annoGroups.Length();g++)
   {
	 ...
	 if(af==_NAN_ ||af==0.0||af==1.0|| singleP==_NAN_)
	 {
	    del.Push(m);
