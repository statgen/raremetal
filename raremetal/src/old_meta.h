...
// /net/fantasia/home/yjingj/METAL/1KG/MAF_1KG.txt
	...
	std::map< String, Vector > variant_fk; //fk of each variant for each study. will turn into fk-tilda if --normPop
	std::map< String, Vector > variant_nk; // nk of each variant for each study
...
//	StringDoubleHash regressedTotalAF; // for pop stratification, update f-tilda
	std::map<String, Vector > af1KG; // 1000g pop af for all markers...
