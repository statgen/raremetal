...
void compute_d(Vector &d, Matrix &J, Vector &np);

void update_z(Vector &z, Matrix &J, Vector &d, int iq);

void update_r(Matrix &R, Vector &r, Vector &d, int iq);

bool add_constraint(Matrix &R, Matrix &J, Vector &d, int &iq, double &rnorm);

void delete_constraint(Matrix &R, Matrix &J, Vector &A, Vector &u, int n, int p, int &iq, int l);

// Utility functions for computing the Cholesky decomposition and solving
// linear systems
void cholesky_decomposition(Matrix &A);

void cholesky_solve(Matrix &L, Vector &x, Vector &b);

void forward_elimination(Matrix &L, Vector &y, Vector &b);

void backward_elimination(Matrix &U, Vector &x, Vector &y);
...
double solve_quadprog(Matrix &G, Vector &g0, Matrix &CE, Vector &ce0, Matrix &CI, Vector &ci0, Vector &
x)
{
    ...
    if (ce0.Length() != p)
    {
        msg << "The vector ce0 is incompatible (incorrect dimension " << ce0.Length() << ", expecting " << p << ")";
        throw std::logic_error(msg.str());
    }
    if (CI.rows != n)
    {
    ...
    if (ci0.Length() != m)
    {
        msg << "The vector ci0 is incompatible (incorrect dimension " << ci0.Length() << ", expecting " << m << ")";
        throw std::logic_error(msg.str());
    }
    x.Dimension(n);
    int i, j, k, l; /* indices */
    ...
    R.Dimension(n, n);
    J.Dimension(n, n);
    Vector s, z, r, d, np, u, x_old, u_old;
    s.Dimension(m + p);
    z.Dimension(n);
    r.Dimension(m + p);
    d.Dimension(n);
    ...
    * and the full step length t2 */
    ...
    for (i = 0; i < p; i++)
    {
        ...
        /* compute full step length t2: i.e., the minimum step in primal space s.t. the contraint
          becomes feasible */
        t2 = 0.0;
        if (fabs(z.InnerProduct(z)) > std::numeric_limits<double>::epsilon())
        { // i.e. z != 0
            t2 = (-np.InnerProduct(x) - ce0[i]) / z.InnerProduct(np);
        }
        ...
        f_value += 0.5 * (t2 * t2) * z.InnerProduct(np);
        A[i] = -i - 1;
    ...
    if (fabs(z.InnerProduct(z)) > std::numeric_limits<double>::epsilon()) // i.e. z != 0
    {
        t2 = -s[ip] / z.InnerProduct(np);
        if (t2 < 0)
    ...
    f_value += t * z.InnerProduct(np) * (0.5 * t + u[iq]);
    /* u = u + t * [-r 1] */
    for (k = 0; k < iq; k++)
        u[k] -= t * r[k];
    u[iq] += t;
...
bool add_constraint(Matrix &R, Matrix &J, Vector &d, int &iq, double &R_norm)
{
    int n = d.Length();
#ifdef TRACE_SOLVER
...
void delete_constraint(Matrix &R, Matrix &J, Vector &A, Vector &
u, int n, int p, int &iq, int l)
{
#ifdef TRACE_SOLVER
...
void cholesky_solve(Matrix &L, Vector &
x, Vector &
b)
{
    int n = L.rows;
...
void print_matrix(const char *name, Matrix &A, int n, int m)
{
    ...
    t = s.str();
    t = t.substr(0, t.length() - 3); // To remove the trailing space, comma and newline
...
void print_vector(const char *name, Vector &v, int n)
{
    ...
    t = s.str();
    t = t.substr(0, t.length() - 2); // To remove the trailing space and comma
