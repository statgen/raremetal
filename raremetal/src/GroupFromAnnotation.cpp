    ...
    while (!feof(samefile))
    {
        ...
        for (int i = 1; i < tmp.Length(); i++)
        {
            ...
            if (sub.Length() != 4)
            {
                ...
                fprintf(log,
                        "Warning: group %s has a variant %s that has invalid format. The correct format should be chr:pos:allele1:allele2.\n",
                        tmp[0].c_str(), tmp[i].c_str());
                continue;
            }
...
// check each alternative field
    ...
    for (int a = 0; a < alts.Length(); a++)
    {
        ...
        { // no pattern to match: check if intergenic first
...
void GroupFromAnnotation::addGeneFromVcf(StringArray &vfield, String &genename)
{
    ...
    markername += ":";
    markername += vfield[4];
    if (SNPlist[gene].FastFind(markername) == -1)
