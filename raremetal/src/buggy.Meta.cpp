...
void Meta::PoolSummaryStat(GroupFromAnnotation & group,FILE * log)
{
   ...
   {
      //hash and variables to check duplicates later
	 ...
	 //TODO:check if this position has been hashed
	 //if yes, then check allele match
	 //if match, continue
	 //if no match, exclude and generate warning

	 //if not hashed, then 
	 //if polymorphic then hash the position and ref/alt alleles 
	 //if monomorphic then 
	 ...
	 {
//check if the position has been hashed
//if yes
  //check if this site is monomorphic
    //if yes, then update N and continue
    //if not, then exclude and generate warning
	    ...
	    if(refalt_saved != NULL)
{
	 ...
	 if((tokens[2]=="." || tokens[3]==".") && (c1+c2!=0 && c2+c3!=0))
	 {
	    printf("Warning: variant %s:%s:%s:%s from study %s is excluded: at least one of the alleles is missing but allele counts show polymorphism.\n",tokens[0].c_str(),tokens[1].c_str(),tokens[2].c_str(),tokens[3].c_str(),studies[study].c_str());
	    fprintf(log,"Warning: variant %s:%s:%s:%s from study %s is excluded: at least one of the alleles is missing but allele counts show polymorphism.\n",tokens[0].c_str(),tokens[1].c_str(),tokens[2].c_str(),tokens[3].c_str(),studies[study].c_str());
	    continue;
	 }

	 //QC: filtering variants
	 //TODO: add the filter where genotyping failure is flagged
	 if(tokens[8-adjust].AsDouble()<CALLRATE || tokens[9-adjust].AsDouble()<HWE)
	 {
	    ...
	    else if(tokens[2]!=tokens[3])
	    {
	       ...
	       if(refalt_saved != NULL)
	       {
		  //if this position has been hashed with different allele then skip
		  skip_count++;
		  String skip_SNP;
		  skip_SNP = study;
		  skip_SNP += ":";
		  ...
		  fprintf(log,"Warning: variant %s:%s:%s:%s from study %s is excluded from analysis due to un-matched alleles from previous studies.\n",tokens[0].c_str(),tokens[1].c_str(),tokens[2].c_str(),tokens[3].c_str(),studies[study]);
	    ...
	    printf("Warning: variant %s:%s:%s:%s from study %s is excluded: did not pass the HWE or CALLRATE filter; direction is coded as '!'.\n",tokens[0].c_str(),tokens[1].c_str(),tokens[2].c_str(),tokens[3].c_str(),studies[study].c_str());
	    fprintf(log,"Warning: variant %s:%s:%s:%s from study %s is excluded: did not pass the HWE or CALLRATE filter; direction is coded as '!'.\n",tokens[0].c_str(),tokens[1].c_str(),tokens[2].c_str(),tokens[3].c_str(),studies[study].c_str());
	    continue;
	 }

	 //Hash the variant for removing duplicates later
	 //if coming to a new chromosome then reset the hash
	 ...
	 if(hashToRemoveDuplicates.Integer(tmp_SNP) ==-1)
	 {
	    if(hashToRemoveDuplicates.Integer(tmp_SNP_flip) ==-1)
	    {
	       varCount++;
	 ...
	 //update adjustment to total sample size
	 int N_idx;
	 if(index1==-1 && index2==-1)
