...
void KinshipEmp::SetupEmpKin(Pedigree &ped, IntArray &genotypedSamplePED, IntArray &genotypedSampleVCF,
                             StringIntHash &samplePEDIDHash, StringIntHash &skippedSNPs, StringArray &chromosomeVCF,
                             FILE *log)
{
#ifdef _OPENMP
...
void KinshipEmp::SetupEmpKinX(Pedigree &ped, IntArray &genotypedSamplePED, IntArray &genotypedSampleVCF,
...
void KinshipEmp::SetupPEDAuto(Pedigree &ped, IntArray &genotypedSamplePED, FILE *log)
{
    ...
    for (int i = 0; i < ped.markerCount; i++)
    {
        ...
        if (markerName.Length() <= 1)
        {
            ...
            fprintf(log,
                    "WARNING: marker name %s in DAT file is not in CHR:POS format and will be skipped from calculating empirical kinship.\n",
                    ped.markerNames[i].c_str());
            continue;
        }
...
void
KinshipEmp::WriteKinship(Pedigree &ped, Matrix &allPairs, IntArray &genotypedSample, bool AUTO, bool VCF, FILE *log)
{
    ...
    for (int i = 0; i < ped.markerCount; i++)
    {
       ...
       for(int p=0;p<total_n;p++)
       {
        ...
        for (int p = 0; p < total_n; p++)
        {
            ...
            {
                if (pos >= PreMeta::Xstart && pos <= PreMeta::Xend &&
                    ped[genotypedSamplePED[p]].sex == PreMeta::maleLabel)
                {
                    X[p] = 2.0;
                } else
                {
...
void KinshipEmp::SetupVCFX(Pedigree &ped, IntArray &genotypedSampleVCF, StringIntHash &samplePEDIDHash,
                           StringIntHash &skippedSNPs, FILE *log)
{
    printf("Calculating empirical kinshipX matrix ...");
    fflush(stdout);
    fprintf(log, "Calculating empirical kinshipX matrix ...");
                ...
                if (ped[ped_idx].sex == PreMeta::maleLabel)
                {
                    ...
                    if (ploidy == 1)
                    {
                        if (std::isnan(geno_tmp[0]))
                        {
                            ...
                            if (nmiss > NMISS)
                            {
                                ...
                                warnings++;
                                fprintf(log,
                                        "Warning: variant %s is skipped when calculating kinshipX, because genotype missing rate is >%g.\n",
                                        SNPname.c_str(), miss);
                                if (warnings < 20)
                                {
                    ...
                    } else
                    {
                        ...
                        } else if (geno_tmp[0] != geno_tmp[1])
                        {
                            ...
                            warnings++;
                            fprintf(log,
                                    "Warning: genotype of sample %s of variant %s is set to be missing because male genotype on non-pseudo-autosomal region of chr X can not be heterozygous.\n",
                ...
                } else
                {
                    for (int j = 0; j < ploidy; j++)
                    {
                        ...
                        if (a > 1)
                        {
                            ...
                            warnings++;
                            fprintf(log,
                                    "Warning: variant %s is skipped when calculating kinshipX, because marker is not bi-allelic.\n",
                                    SNPname.c_str());
                            if (warnings < 20)
                            {
