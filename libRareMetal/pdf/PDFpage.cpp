...
void PDFPage::DrawCheck(double x, double y, double width, double height, double ratio, double line_width)
   {
   SetLineWidth(line_width);
   DrawLine(x - 0.5 * width, y , x - 0.3 * width, y - 0.55 * height - line_width * 0.5);
   DrawLine(x - 0.3 * width, y - 0.5 * height, x + 0.5 * width, y + 0.5 * height);
   }
...
void PDFPage::AddHeader(const char * title, const char * sub_title, int title_fontsize, int sub_fontsize)
   {
   hTextAlignment = taCenter;
   vTextAlignment = taMiddle;

   SetFontSize(title_fontsize);
   WriteText(GetWidth() * 0.5, GetHeight() * 0.95, title);

   SetFontSize(sub_fontsize);
   WriteText(GetWidth() * 0.5, GetHeight() * 0.91, sub_title);
   }

void PDFPage::AddFootNote(double x_pct_offset, const char * note, double y_pct_offset, int font_size)
   {
   hTextAlignment = taLeft;
   vTextAlignment = taMiddle;

   SetFontSize(font_size);
   WriteText(GetWidth() * x_pct_offset, GetHeight() * y_pct_offset, note);
   }
